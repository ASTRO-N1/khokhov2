...
import { Match, User as UserType, ScoringAction, Team } from "../../types";
...
// Define a type for the data needed to display the full report
...
export function ScorerHomeEnhanced({
  ...
  // NEW STATE: Holds all data needed for the final report modal
  ...
  useEffect(() => {
    const fetchAssignedMatches = async () => {
      ...
      // We select all necessary match data fields, including toss details and player count
      ...
      if (error) {
        ...
        const formattedMatches: Match[] = data.map((m: any) => ({
          ...
          teamA: {
            ...
            // Ensure players array has required fields for later filtering
          ...
          // Store the toss/setup details directly on the match object (necessary for reconstruction)
  ...
  // NEW FUNCTION: Fetches all necessary data to display the Consolidated Report
  const handleViewResults = async (match: Match) => {
    setLoading(true);

    // 1. Fetch All Scoring Actions (crucial for report accuracy)
    ...
    if (actionsError) {
      toast.error("Failed to fetch match actions: " + actionsError.message);
      setLoading(false);
      return;
    }

    // 2. Reconstruct MatchSetupData from the saved Match record fields
    const reconstructedSetupData: MatchSetupData = {
      // For a finished match report, we treat the entire squad as available for lookups
      ...
      playersPerTeam: (match.playersPerTeam as 7 | 9) || 7,
      timerDuration: match.turnDuration || 540,
    };

    // 3. Set the report data and open the modal
    setReportData({
      match,
      actions: actionsData as ScoringAction[],
  ...
  return (
    <>
      {/* Renders the modal with full report data */}
      {reportData && (
        <ViewResultModal
          match={reportData.match}
          onClose={handleCloseReport}
          actions={reportData.actions}
          setupData={reportData.setupData}
        />
      )}
